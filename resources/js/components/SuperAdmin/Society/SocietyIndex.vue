<template>
 ...
 <div class="">
  <button class="bg-white font-semibold py-2 px-4 border border-gray-400 relative rounded hover:shadow-lg hover:shadow-blue-500/50 shadow-md mt-3 ml-4 ">
      <router-link  class="flex text-blue-500 hover:text-blue-600" :to="{name: 'create.society.superadmin'}">
        <p class="my-auto ml-2">
          Subscrire a new society
        </p>
                ...
                <template v-for="society in societies" :key="society.id">
                    <tr class="bg-white border-b">
                        ...
                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap flex">
                            <router-link :to="{name: 'edit.society.superadmin', params : {id : society.id}}" > 
                            ...
                            <svg @click="deleteSociety(society.id)" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-red-400 cursor-pointer" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
import useSociety from '../../../services/societyservices.js';
export default {
    setup(){
      const toSearch = ref('');
        const { getSocieties, societies, destroySociety, searchSociety} = useSociety();

        onMounted(getSocieties());

        const deleteSociety = async (id) => {
            await destroySociety(id);
        }

        const search = async () => {
          if(toSearch.value.trim() !== ''){
              await searchSociety(toSearch.value.trim())
          }
          else {
            getSocieties();
          }
