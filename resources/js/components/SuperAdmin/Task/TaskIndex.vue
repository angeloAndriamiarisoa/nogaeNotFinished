<template>
    <!-- <button class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow">
        <router-link :to="{name: 'create.task.superadmin'}">Create</router-link>
    </button>
    <h1>Index of task</h1> -->
       <!-- create button -->
                ...
                <template v-for="task in tasks" :key="task.id">
                    <tr class="bg-white border-b">
                        ...
                        <td class="text-sm text-gray-900 font-light px-6 py-4 flex">
                            <router-link :to="{name: 'edit.task.superadmin', params : {id : task.id}}"> 
                            <!-- editsvg  -->
...
export default {
    setup() {
      const toSearch = ref('');
      const { destroyTask, getTasks, tasks, searchSuper } = useTask();
        ...
        const search = async () => {
          if(toSearch.value.trim() !== ''){
              await searchSuper(toSearch.value.trim());
          }
          else {
            getTasks();
          }
